...
def path_length(graph, path):
    ...
    cur=path[0]
    for x in path[1:]:
        if graph.is_neighbor(cur,x):
            ans+=graph.get_edge(cur,x).length
        cur=x
    return ans
...
def extensions(graph, path):
    ...
    x = path[-1]
    res = []
    for y in sorted(graph.get_neighbors(x)):
        new = path+[y]
        if not has_loops(new):
            res.append(new)
    return res
...
def basic_dfs(graph, startNode, goalNode):
    ...
    while len(agenda)>0:
        ...
        if cur == goalNode:
            return path
        agenda[:0] = extensions(graph, path)
    return None
...
def basic_bfs(graph, startNode, goalNode):
    ...
    while len(agenda)>0:
        ...
        if cur == goalNode:
            return path
        agenda.extend(extensions(graph, path))
    return None
...
def hill_climb_sort(graph, goalNode, new_paths):
    new_paths = break_ties(new_paths)
    return sorted(new_paths, 
            key = lambda path:graph.get_heuristic_value(path[-1], goalNode))

def branch_and_bound_sort(graph, goalNode, new_paths):
    new_paths = break_ties(new_paths)
    return sorted(new_paths, 
            key = lambda path:path_length(graph, path))

def branch_and_bound_sort_with_heuristic(graph, goalNode, new_paths):
    new_paths = break_ties(new_paths)
    return sorted(new_paths, 
            key = lambda path:
...
generic_best_first = [do_nothing_fn, False, hill_climb_sort, False]

generic_branch_and_bound = [do_nothing_fn, False, branch_and_bound_sort, False]

generic_branch_and_bound_with_heuristic = [do_nothing_fn, False, branch_and_bound_sort_with_heuristic, False]

generic_branch_and_bound_with_extended_set = [do_nothing_fn, False, branch_and_bound_sort, True]

generic_a_star = [do_nothing_fn, False, branch_and_bound_sort_with_heuristic, True]
...
def my_beam_sorting_fn(graph, goalNode, paths, beam_width):
    if len(paths) == 0:
        #print(paths)
        return paths
    check_len = max(len(paths[0]),len(paths[-1])-1)
    news = [x for x in paths if len(x)==check_len]
    if len(news)<=beam_width:
        #print(paths)
        return paths
    res = [x for x in paths if len(x)<check_len]
    res = res + hill_climb_sort(graph, goalNode, news)[:beam_width]
    res = res + [x for x in paths if len(x)>check_len]
    #print(res)
    return res
...
def is_admissible(graph, goalNode):
    ...
    for x in graph.nodes:
        if graph.get_heuristic_value(x, goalNode) \
            > path_length(graph, generic_search(*generic_branch_and_bound_with_extended_set)(graph, x, goalNode)):
            adm = False
    return adm
...
def is_consistent(graph, goalNode):
    ...
    for ed in graph.edges:
        x=ed.startNode
        y=ed.endNode
        if abs(graph.get_heuristic_value(x, goalNode)- graph.get_heuristic_value(y, goalNode))\
                > ed.length:
            adm = False
    return adm
