...
def primes_up_to(x):
    ...
    return [y for y in range(int(x)+1) if is_prime(y)]
...
def expression_depth(expr):
    ...
    if type(expr) is list:
        res = 0
        for x in expr[1:]:
            res = max(res, expression_depth(x))
        return res+1
    else:
        return 0
...
def remove_from_string(string, letters):
    ...
    return ''.join([c for c in string if not c in list(letters)])
...
def compute_string_properties(string):
    ...
    """
    return len(string), list(reversed(sorted(string))), len(set(string))
...
def create_length_comparer_function(check_equal):
    ...
    lengths."""
    return lambda a,b: (not ((len(a)==len(b)) ^ check_equal))
...
def sort_points_by_Y(list_of_points):
    ...
    return sorted(list_of_points, key = lambda p:-p.getY())
...
def furthest_right_point(list_of_points):
    ...
    return sorted(list_of_points, key = lambda p:-p.getX())[0]
